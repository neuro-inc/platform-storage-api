...
gke_k8s_deploy_dev:
	...
	helm --set "global.env=dev" --set "values.IMAGE=$(IMAGE_K8S):$(CIRCLE_SHA1)" upgrade platformstorageapi deploy/platformstorageapi/ --wait --timeout 600
...
gke_k8s_deploy_staging:
	...
	sudo chown -R circleci: $(HOME)/.kube
	helm --set "global.env=staging" --set "values.IMAGE=$(IMAGE_K8S):$(CIRCLE_SHA1)" upgrade platformstorageapi deploy/platformstorageapi/ --wait --timeout 600
