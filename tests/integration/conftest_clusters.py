            ...
            try:
                ...
                if (pod.status.phase or "") == "Running":
                    ...
                    if container_statuses and all(
                        status.ready and status.state.terminated is None
...
async def _get_vcluster_service_port(
    ...
    if not ports or ports[0].port is None:
        err_message = "vcluster service has no ports configured"
...
async def _start_port_forward(
    ...
    try:
        ...
        process.terminate()
        await process.wait()
        err_message = "port-forward timeout"
        raise RuntimeError(err_message) from None
