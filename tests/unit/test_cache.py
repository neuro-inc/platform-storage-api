...
async def test_permission_cache() -> None:
    ...
    async def _get_user_permissions_tree(
        request: Request, target_path: PurePath
    ...
    async def get_user_permissions_tree(
        ...
        call_log.append(("tree", target_path))
        return copy.deepcopy(await _get_user_permissions_tree(request, target_path))

    async def check_user_permissions(
        request: Request, target_path: PurePath, action: str
    ) -> None:
        call_log.append(("check", target_path))
        tree = await _get_user_permissions_tree(request, target_path)
        if not _has_permissions(action, tree.action):
            raise HTTPNotFound

    cache = PermissionsCache(
        get_user_permissions_tree=get_user_permissions_tree,
